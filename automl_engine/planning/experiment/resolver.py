...
from automl_engine.data import infer_task, apply_leakage_policy
from automl_engine.evaluation import resolve_metric, get_cv_object
from automl_engine.planning.models.registry import MODEL_REGISTRY, COST_LOW, COST_MEDIUM
...
class ExperimentResolver:
    ...
    def resolve(
        ...
        # Users commonly mutate nested dataclass fields after construction.
        # Re-run validation here so invalid mutations fail before training.
        ...
        data_info: DataInfo = DataInfo.from_data(X, y, task=task)
    ...
    def _filter_models(
        ...
        unknown_includes = set(ms_config.include_models or []) - set(models)
        unknown_excludes = set(ms_config.exclude_models or []) - set(models)
        if unknown_includes:
            raise ValueError(f"Unknown models in include_models: {sorted(unknown_includes)}")
        if unknown_excludes:
            raise ValueError(f"Unknown models in exclude_models: {sorted(unknown_excludes)}")

        if ms_config.include_models:
            models = {name: info for name, info in models.items() if name in ms_config.include_models}

        if ms_config.exclude_models:
            models = {name: info for name, info in models.items() if name not in ms_config.exclude_models}

        allow_missing_via_pipeline = config.preprocessing.imputation_strategy != "none"
        models = {
            name: info
            for name, info in models.items()
            if is_model_suitable(
                info,
                data_info,
        ...
        if s_config.compute_budget == "low":
            models = {name: info for name, info in models.items() if info.training_cost == COST_LOW}
